...
def deleteAbove(input):
    df = input
    surface = df[df["class"] == 5]
    surface_elev = np.mean(surface["elev"])
    good = df[(df["elev"] < surface_elev) | (df["class"] == 5)]
    return good
...
def aboveLand(input):
    ...
    surface = df[df["class"] == 5]
    surface_elev = np.mean(surface["elev"])
    df.loc[(df['elev'] > surface_elev) & (df['class'] != 5), 'class'] = 9
    return df


# File structure
# dataDir = "D:/Anders/SDB/icesat/data/7185/"
# dataDir = "../test_data2/"
# dataDir = "D:/Yiwen/icesat2/ATL03_20181101191203_05220102_005_01"
...
parser.add_argument('--maxElev', type=int, default=20, help='Maximum elevation for filter')
...
def main(args):
    ...
    for filename in filenames:
        ...
        dir = os.path.join(os.path.dirname(filename), os.path.basename(filename)[:-3])
        ...
        with h5py.File(filename, mode='r') as fileID:
            ...
            for gtx in IS2_atl03_beams:
                ...
                # df['dist_ph_across'] = IS2_atl03_mds[gtx]['heights']["dist_ph_across"]
                # df['dist_ph_along'] = IS2_atl03_mds[gtx]['heights']["dist_ph_along"]
                ...
                # df['pce_mframe_cnt'] = IS2_atl03_mds[gtx]['heights']["pce_mframe_cnt"]
                # df['ph_id_channel'] = IS2_atl03_mds[gtx]['heights']["ph_id_channel"]
                # df['ph_id_count'] = IS2_atl03_mds[gtx]['heights']["ph_id_count"]
                # df['ph_id_pulse'] = IS2_atl03_mds[gtx]['heights']["ph_id_pulse"]
                ...
                if args.removeIrrelevant:
                    df_data = df_data[(df_data["elev"] > args.minElev) & (df_data["elev"] < args.maxElev)]

                if args.removeLand:  # Remove photons for which the DEM is more than 50m above the geoid
                ...
                # Round to three decimals for all variables - removed
                # df = df.round(decimals=3)

                # Find water surface
                df = findSurface(df, args.minElev, args.maxElev)

                # Delete points above water surface - consider re-instating this
                # df = deleteAbove(df)

                # Instead of deleting those points, set them to 9 (land)
                df = aboveLand(df)
                ...
                if args.utm:
                    outFilename = dir + "/" + os.path.basename(filename)[
                                              :-3] + "_" + gtx + "_raw_" + zone + ".csv"  # Reinstate if
                else:
                    outFilename = dir + "/" + os.path.basename(filename)[:-3] + "_" + gtx + "_raw" + ".csv"
